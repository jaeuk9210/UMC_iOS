...
//        setBinding()
    ...
    @objc func loginButtonDidTap(_ sender: UIButton) {
        LoginDataManager().login(self, LoginInput(email: email, password: password))
    }
...
extension LoginViewController {
    ...
    func successLoginAPI(_ result: LoginModel) {
        if result.isSuccess ?? false {
            UserDefaults.standard.set(result.result?.token, forKey: "authKey")
            updateLoginStatus()
